...
#                  output_dir='./basic-scRNA-Seq'):
    ...
    def download_dataset(self):
        click.echo(f'> Downloading dataset ' + click.style(f'{self.dataset}', bold=1) + '..')
        scarf.fetch_dataset(self.dataset, save_path=self.output_dir)

    def read_data(self):
        reader = scarf.CrH5Reader(f'{self.output_dir}/{self.dataset}/data.h5', file_type='rna')
        zarr_fn = f'{self.output_dir}/data.zarr'
        click.echo(f'> Creating zarr file from dataset, at ' + click.style(click.format_filename(f'{zarr_fn}'), bold=1) + '..')
...
#                 highs=[15000, 4000, 15], lows=[1000, 500, 0])
    ...
    def save_data(self):
        ...
        self.ds.export_markers_to_csv(group_key='RNA_cluster', csv_filename=f'{self.output_dir}/markers.csv')
